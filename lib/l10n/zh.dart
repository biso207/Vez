const Map<String, String> zh = {
  // authentication (login & signup) //
  "login": "我回来了",
  "signup": "我是新人",
  "choose_account_type": "选择账号类型",
  "choose_account_type_hint": "选择你如何使用 Vez",
  "account_type_user": "用户",
  "account_type_user_hint": "For Personal Events",
  "account_type_venue": "场所",
  "account_type_venue_hint": "Promote your Business",
  "venue_signup_title": "Venue Signup",
  "venue_signup_subtitle": "Host your next event",
  "venue_name": "场所名称",
  "venue_legal_name": "公司名称",
  "venue_vat_number": "税号/VAT",
  "venue_address": "地址",
  "venue_country": "IT",
  "venue_public_email": "场所邮箱",
  "venue_public_phone": "场所电话",
  "venue_links_hint": "如有可添加公开链接",
  "venue_website": "网站",
  "venue_instagram": "Instagram",
  "venue_pending_title": "验证中",
  "venue_pending_body": "我们会审核场所资料。若可通过，你会收到邮件验证码。",
  "venue_code_confirmed": "验证码已确认，等待最终审核。",
  "venue_code_invalid": "验证码无效",
  "venue_filter_locked": "场所只能查看自己的活动",
  "venue_notifications_disabled": "场所不能被邀请参加活动",
  "top_title_signup": "欢迎来到 Vez",
  "under_title_signup": "创建并享受现实生活中的活动",
  "top_title_login": "嘿！",
  "under_title_login": "欢迎回到 Vez",
  "username": "用户名",
  "password": "密码",
  "phone": "电话",
  "date_of_birth": "出生日期",
  "set_city": "设置城市",
  "locating_the_city": "Locating City...",
  "user_signup_title": "User Signup",
  "user_signup_subtitle": "Create, Explore and Enjoy Events",
  "otp_code": "OTP Code",
  "resend_otp_code": "Resend OTP Code",
  "cancel_signup_title": "Cancel Signup?",
  "cancel_signup_message": "Your signup data will be deleted.",
  'blocked_account': "账号已封锁",
  'this_account_is_blocked': "此账号已被封锁",
  'write_to': "写信给",
  'request_unlock_account': "申请解锁",

  // errors //
  "fill_all_fields": "请填写所有字段",
  "invalid_credentials": "无效的凭据",
  "choose_username": "请选择用户名",
  "choose_profile_photo": "请选择头像",
  "username_too_short": "用户名太短（至少 3 个字符）",
  "user_already_exists": "用户已存在",
  "same_username": "新用户名与旧用户名相同",
  "invalid_email": "无效的电子邮件",
  "invalid_password": "密码无效。\n需要 8 个字符、1 个大写字母、1 个小写字母、1 个数字、1 个特殊字符。",
  "signup_failed": "注册失败",
  "signup_successful": "注册成功！",
  "invalid_phone": "Invalid Phone",
  "otp_request_failed": "OTP Request Failed",
  "otp_invalid": "Invalid OTP Code",
  "otp_expired": "OTP Code Expired",
  "something_went_wrong": "Error",
  "enable_location_services": "请开启位置服务",
  "location_permissions_denied": "位置权限被拒绝",
  "location_permissions_permanently_denied": "位置权限已被永久拒绝，请在设置中更改",
  "unknown_city": "未知城市",
  "no_internet_connection": "无网络连接",
  "login_failed": "登录失败",

  // global //
  "search": "搜索",
  "filter_by_you": "我的",
  "filter_invited": "邀请",
  "filter_nearby": "附近",
  "all": "全部",
  "going": "参加",
  "not_going": "不参加",
  "maybe": "待定",
  "guest_list": "宾客",
  "search_guest": "搜索",
  "host": "主办方",
  "no_guests_yet": "暂无宾客",
  "no_guests_found": "没有可邀请的人",
  "guest_remove_failed": "无法移除宾客",
  "add_guest": "邀请",
  "friends": "好友",
  "following": "关注",
  "anyone": "任何人",
  "guest_add_failed": "无法添加宾客",
  "guest_invites_private_only": "宾客仅适用于私人或专属活动",
  "no_events_by_you": "暂无活动",
  "no_events_invited": "暂无邀请",
  "no_events_nearby": "暂无附近活动",
  "follow": "关注",
  "follow_back": "回关",
  "unfollow": "取消关注",
  "follow_failed": "关注失败",
  "unfollow_failed": "取消关注失败",
  "user_not_found": "未找到用户",

  // edit profile //
  "edit_username": "用户名",
  "new_password": "新密码",
  "city_aka_name": "城市别名",
  "bio": "个人简介",
  "category_badge": "分类徽章",
  "city": "城市",

  // settings //
  "settings": "设置",
  "display": "显示",
  "account": "账户",
  "click_to_change": "点击更改",
  "logout": "退出登录",
  "close": "关闭",
  "change_password": "Change Password",
  "current_password": "Current Password",
  "confirm_password": "Confirm Password",
  "complete_all_fields": "Complete All Fields",
  "passwords_do_not_match": "Passwords Do Not Match",
  "wrong_current_password": "Wrong Current Password",
  "password_change_failed": "Password Change Failed",
  "delete_account": "Delete Account",
  "delete_account_confirm_message":
      "Type {username} To Delete Your Account Forever.",
  "username_confirm_failed": "Username Does Not Match",
  "account_delete_failed": "Account Delete Failed",

  // event creation page //
  "event_title": "标题",
  "date": "日期",
  "time": "时间",
  "location": "地点",
  "details": "详情",
  "max_guests": "最多宾客",
  "price": "价格",
  "preview": "预览",
  "edit_bg": "编辑背景",

  // setter event details //
  "save_event": "保存活动？",
  "delete_data": "删除数据？",
  "set_location": "选地点",
  "set_details": "详情",
  "set_max_guests": "最多宾客",
  "set_price": "价格",
  "location_simple_name": "地点名称",
  "location_map": "地图",
  "location_selector_hint": "选择地点保存方式",
  "location_simple_name_hint": "仅名称，无坐标",
  "location_map_nearby_hint": "使用坐标，Nearby 需要",
  "move_map_to_select_place": "移动地图",
  "unknown_place": "未知地点",
  "address_not_found": "未找到地址",
  "network_error_try_again": "网络错误，请重试",
  "confirm_location": "确认位置",
  "confirm": "是",
  "cancel": "否",
  "delete_event": "删除",
  "event_saved_success": "活动已成功保存",
  "event_save_failed": "无法保存活动",
  "event_place_save_failed": "无法保存地点",
  "event_updated_success": "活动已更新",
  "event_update_failed": "无法更新活动",
  "event_deleted_success": "活动已删除",
  "event_delete_failed": "无法删除活动",
  "no_price": "无价格",
  "no_limit": "无限制",

  // event typologies //
  "exclusive": "专属",
  "private": "私人",
  "public": "公开",

  // event categories //
  "cinema": "电影院",
  "concert": "音乐会",
  "disco": "夜店",
  "gaming": "游戏",
  "hang_out": "聚会",
  "journey": "旅行",
  "kids_and_family": "亲子活动",
  "museum": "博物馆",
  "outdoor": "户外",
  "party": "派对",
  "pub": "酒吧",
  "restaurant": "餐厅",
  "shopping": "购物",
  "sport": "运动",
  "theatre": "剧院",
  "wellness": "健康",
  "workshop": "工作坊",

  // language selection //
  "select_language": "选择语言",
  "lang_en": "英语",
  "lang_it": "意大利语",
  "lang_zh": "中文",
  "lang_fr": "法语",
  "lang_de": "德语",
  "lang_es": "西班牙语",

  // notifications //
  "type1": "{name}，来自 {host} 的邀请：{event}。",
  "type2": "{host} 邀请你参加 {event}。",
  "type3": "新邀请：{event}。",
  "type4": "{name}，你被邀请参加 {event}。",
  "type5": "收到来自 {host} 的邀请。",
  "type6": "{event} 在等你。",
  "type7": "{host} 想到了你。",
  "type8": "为你准备的新活动：{event}。",
  "type9": "{name}，看看这个：{event}。",
  "type10": "邀请来了 👀 {event}",
};
