...
class SignupFlowController extends ChangeNotifier {
  ...
  // requests a fresh otp record from the backend.
  Future<bool> requestOtp() async {
    ...
    loading = true;
    notifyListeners();
    final int result = await _db.requestSignupOtp(
      phone: phoneController.text.trim(),
      accountType: accountType.dbValue,
    );
    loading = false;

    if (result == 200 || result == 201) return true;
    error = '${StringRes.at("otp_request_failed")}\n$result';
    notifyListeners();
    return false;
  }

  // verifies the otp and persists the account data.
  Future<int> completeSignup() async {
    ...
    final int otpResult = await _db.verifySignupOtp(otpController.text.trim());
    if (otpResult == 410) {
      loading = false;
      error = StringRes.at('otp_expired');
      notifyListeners();
      return otpResult;
    }
    if (otpResult != 200) {
    ...
    final int signupResult = await _db.signupAuthFlow(
      accountType: accountType.dbValue,
    ...
    loading = false;
    if (signupResult == 409) {
      error = StringRes.at('user_already_exists');
    } else if (signupResult != 200 && signupResult != 201) {
      error = '${StringRes.at("signup_failed")}\n$signupResult';
    }
    notifyListeners();
    return signupResult;
  }
